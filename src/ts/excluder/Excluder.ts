...
export class Excluder {
	...
	public exclude({ start, end, ...sequenceItem }: ICampaignStructure, playbacks: IPlaybackStructure[]): IResult {
		...
		const createdPlaylistFromEmptyPlaylist: IPlaylistStructure = playlistController.removeFrom(
	...
	private _getAssignation(uniquePlaylists: IPlaylistStructure[], playlistController: PlaylistController): IAssignation {
		...
		for (const playlist of uniquePlaylists) {
			const createdPlaylist: IPlaylistStructure = playlistController.removeFrom(playlist);
